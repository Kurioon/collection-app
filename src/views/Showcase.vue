...
// Функція для завантаження наступної "сторінки" предметів
const loadMore = () => {
  itemsStore.fetchPublicItems(true)
}
</script>
  ...
  <div class="min-h-screen pb-20">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div v-else>
        ...
        <div v-if="itemsStore.hasMore" class="mt-12 flex justify-center">
          <button @click="loadMore" :disabled="itemsStore.loading" class="glass-btn !w-auto px-8">
            {{ itemsStore.loading ? $t('showcase.loading_more') : $t('showcase.load_more_btn') }}
          </button>
        </div>
      </div>
    </main>
